...
def check_designer(base: str) -> None:
    ...
    resp.raise_for_status()
    if "designer" not in resp.url and resp.status_code != 200:
        raise RuntimeError("designer page did not load")


def check_logs(base: str) -> None:
    # The new app serves events.log by default; just verify the endpoint responds successfully.
    resp = requests.get(urljoin(base, "/logs"), params={"name": "events.log"}, timeout=TIMEOUT)
    resp.raise_for_status()
